...
export default function VideoModal({
  ...
  // Reset state when modal opens/closes or variant changes
  useEffect(() => {
    if (isOpen) {
      setViewMode(variant);
    }
      ...
      <div className={`bg-white rounded-2xl overflow-hidden w-full max-h-[90vh] flex flex-col ${isClip ? 'max-w-3xl' : 'max-w-5xl'}`}>
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-3 min-w-0">
            {/* Back to clip button when expanded from clip */}
            {!isClip && variant === 'clip' && (
              <button
        ...
        {/* Clip variant: transcription + action bar */}
        {isClip && transcription && (
          <div className="px-4 pt-3 pb-1">
            <p className="text-sm text-gray-500 mb-1 font-medium">Transcript</p>
            <p className="text-sm text-gray-700 italic">&ldquo;{transcription}&rdquo;</p>
          </div>
        )}
        {isClip && (
          ...
          <>
            ...
            <div className="p-4 overflow-y-auto flex-1">
              ...
              <div className="flex items-center gap-3 mt-4 pt-4 border-t">
                {videoId && (
                  <a
                    href={`/analyze?videoId=${encodeURIComponent(videoId)}`}
                    className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
